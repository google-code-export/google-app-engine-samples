...
// Author: ryanb@google.com (Ryan Barrett)

/**
 * @fileoverview
 * Javascript code for the interactive SQL shell for the Google SQL Service.
 *
 * Includes a function (shell.runStatement) that sends the current SQL
...
/**
 * Key code constants for a few keys.
 * @type Number
 */
shell.UP_ARROW = 38;
shell.DOWN_ARROW = 40;
...
shell.onPromptKeyPress = function(event) {
  ...
  if (event.ctrlKey && event.keyCode == this.UP_ARROW) {
    if (this.historyCursor > 0) {
      statement.value = this.history[--this.historyCursor];
    }
    return false;
  } else if (event.ctrlKey && event.keyCode == this.DOWN_ARROW) {
    if (this.historyCursor < this.history.length - 1) {
      statement.value = this.history[++this.historyCursor];
    }
    return false;
...
/**
 * This is the form's onsubmit handler. It sends the SQL statement to the
