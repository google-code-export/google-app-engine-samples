...
def get_greeting():
  ...
  if user:
    ...
    has_voted, has_added_quote = models.get_progress(user.email())    
    if has_voted:
      progress_id |= 4
      progress_msg = ""
    if has_added_quote:
...
class MainHandler(webapp.RequestHandler):
  ...
  def get(self):
    ...
    quotes, next = models.getquotes(page)
    if next:
      nexturi = "/?p=%d" % (page + 1)
    else:
      nexturi = None
  ...
  def post(self):
    ...
    if uri and ( not parsed_uri.scheme or not parsed_uri.netloc ):
      ...
      quoteid = models.addquote(text, user, uri=uri)
      models.setvote(long(quoteid), user, 1)
...
class VoteHandler (webapp.RequestHandler):
  ...
  def post(self):
    ...
    if not vote in ["1", "-1"]:
      ...
      return
    vote = int(vote)
    models.setvote(long(quoteid), user, vote)
...
class RecentHandler(webapp.RequestHandler):
  ...
  def get(self):
    ...
    if not offset:
      offset = None
    quotes, next = models.getquotes_newest(offset)
    if next:
...
class QuoteHandler (webapp.RequestHandler):
  ...
  def post(self, quoteid):
    """Delete a quote."""
    user = users.get_current_user()
    models.delquote(long(quoteid), user)
    self.redirect("/")

  def get(self, quoteid):
    """Get a page for just the quote identified."""
    quote = models.getquote(long(quoteid))
    if quote == None:
      self.response.set_status(404, "Not Found")
      return      
    user = users.get_current_user()
